...
__kernel void linear_interpolation_GPU(__global float *Xv, __global float * Xfirst, __global float* Xsecond, __global float* Vq,__global int * info) {
    int pos = get_global_id(0);


	int XVsize = info[0];
	float item = log10(Xv[pos]);
	for(int i = 0; i< XVsize;i++){
		if(item<Xfirst[i]){
			if(i != XVsize-1){


				float delta = (item - Xfirst[i])/ (Xfirst[i+1] - Xfirst[i]);
