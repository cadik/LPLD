...
__kernel void linear_interpolation_GPU(__global double *Xv, __global double * Xfirst, __global double* Xsecond, __global double* Vq,__global int * info) {
    int pos = get_global_id(0);


	int XVsize = info[0];
	double item =  log10(Xv[pos]);
	for(int i = 0; i< XVsize;i++){
		if(item<Xfirst[i]){
			if(i != XVsize-1){


				double delta = (item - Xfirst[i])/ (Xfirst[i+1] - Xfirst[i]);
				Vq[pos] = delta * Xsecond[i+1] + (1-delta) * Xsecond[i];
				return;
			}
		}
		
	}
	Vq[pos] = Xsecond[XVsize-1];
