    ...
    else
    {
        ...
        for (int r = -HALF_FILTER_SIZE; r <= HALF_FILTER_SIZE; r++)
        {
            ...
            for (int c = -HALF_FILTER_SIZE; c <= HALF_FILTER_SIZE; c++, fIndex++)
            {
                ...
                mu1_1 += (cached_test[ myLocal + curRow + c ] * cached_test[ myLocal + curRow + c ])* filter[ fIndex ];
                mu2_2 += (cached_ref[ myLocal + curRow + c ] * cached_ref[ myLocal + curRow + c ])* filter[ fIndex ];
                mu1_2 += (cached_test[ myLocal + curRow + c ] * cached_ref[ myLocal + curRow + c ])* filter[ fIndex ];
        ...
        real_t sigma1_sq, sigma2_sq, sigma12;
        real_t struc;

        sigma1_sq = max((real_t) 0, mu1_1 - mu1_sq);
        sigma2_sq = max((real_t) 0, mu2_2 - mu2_sq);
        sigma12 = mu1_2 - mu1_mu2;

        struc = (sigma12 + C3) / (sqrt(sigma1_sq) * sqrt(sigma2_sq) + C3);
