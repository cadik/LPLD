...
namespace dip {
    template void SSIM_STRUCT::private_compute<double>(int cv_mat_type) throw (std::runtime_error, cl::Error);
    template void SSIM_STRUCT::private_compute<float>(int cv_mat_type) throw (std::runtime_error, cl::Error);


    SSIM_STRUCT::SSIM_STRUCT(cv::Mat &testImg, cv::Mat &referenceImg, int platformNum, \
    ...
    void SSIM_STRUCT::compute() throw (std::runtime_error) {
    ...
    void SSIM_STRUCT::initCL() throw (std::runtime_error, cl::Error){
...
//             dip::CLInfo::printQuickPlatformInfo(m_platform, m_platformNum);
            ...
            try {
                ...
                m_clSrcFileName = "ssim_struct.cl";
...
//             dip::CLInfo::printQuickDeviceInfo(m_device, m_deviceNum);
    ...
    template <typename T>
    void SSIM_STRUCT::private_compute(int cv_mat_type) throw (std::runtime_error, cl::Error) {
...
//         std::cout << "m_reference_Img.size() = " << m_reference_Img.size() << std::endl;
        ...
        try {
            T L = 1.0;
            T C3 = static_cast<T>((0.5 * std::pow(m_k_2 * L, 2.0)));
...
//             std::cout << compilerOptions << std::endl;
            ...
            cl::Kernel kernel_ssim =  cl::Kernel(m_program, "ssim_struct");
    ...
    double SSIM_STRUCT::fftw_convolution() {
		...
		cv::Mat sig1 = cv::Mat::zeros(m_image_height, m_image_width, CV_64FC1);
		cv::Mat sig2 = cv::Mat::zeros(m_image_height, m_image_width, CV_64FC1);
		cv::Mat sig21 = cv::Mat::zeros(m_image_height, m_image_width, CV_64FC1);

		cv::filter2D(m_test_Img.mul(m_test_Img), sig1, -1, gaussianKernel, cv::Point(-1, -1), 0.0, cv::BORDER_CONSTANT);
		cv::filter2D(m_reference_Img.mul(m_reference_Img), sig2, -1, gaussianKernel, cv::Point(-1, -1), 0.0, cv::BORDER_CONSTANT);
		cv::filter2D(m_test_Img.mul(m_reference_Img), sig21, -1, gaussianKernel, cv::Point(-1, -1), 0.0, cv::BORDER_CONSTANT);
		//GPLib::writeCvMatToFile<double>(sig1, "matrixes/sig1.yml", true);
		sig1 = sig1 - mu1_sqr;
		sig2 = sig2 - mu2_sqr;
		sig21 = sig21 - mu21;

		sig1 = sig1.setTo(0, (sig1)<0);
		sig2 = sig2.setTo(0, (sig2)<0);
		sig21 = sig21.setTo(0, (sig21)<0);
		
		cv::sqrt(sig1, sig1);
		cv::sqrt(sig2, sig2);
		double C3 = 0.5 * std::pow(m_k_2*1.0, 2.0);
		m_result = (sig21 + C3)/(sig1.mul(sig2) + C3);
		return 0.0;
        ...
        for(unsigned int i = 0; i < numOfPixels; i++) {
            double L = 1.0;
            double C3 = 0.5 * std::pow(m_k_2*L, 2.0);
            ...
            double sigma1_sq = std::max<double>(0.0, (out_test_Mul_test[i][0] * scale) - mu1_sq);
            double sigma2_sq = std::max<double>(0.0, (out_ref_Mul_ref[i][0] * scale) - mu2_sq);
            double sigma12   = (out_test_Mul_ref[i][0] * scale) - mu1_mu2;

            double struc = (sigma12 + C3) / (sqrt(sigma1_sq) * sqrt(sigma2_sq) + C3);

            *((double *) m_result.data + i) = struc;
        }
    ...
    unsigned int SSIM_STRUCT::roundUp(unsigned int value, unsigned int multiple) const {
    ...
    int SSIM_STRUCT::nextPowerOf2(int32_t number) const {
        number--;
        number |= number >> 1;
        number |= number >> 2;
        number |= number >> 4;
    ...
    cv::Mat SSIM_STRUCT::normalizeMat(const cv::Mat& inputMat) {
